<script language="JavaScript" type="text/javascript">
    function fn_updateorderstatus(orderid) {


        var result = confirm("Are you sure for Approve Order ?");
        if (result)
        {
            $.ajax({
                type: "POST",
        ...
        <div class="widget-body"> 
            <table id="example-1" cellspacing="0" width="100%" class="table table-hover">
                ...
                <tbody>
                    ...
                    $q = mysqli_query($con, 'SELECT `order_no`,`user_name`,`date`,`is_pending`,`is_shifted`,`is_processing`,`is_completed`,`id`,SUM(qty) as total FROM tbl_order where is_pending=1 and is_shifted=0 and is_processing=0 and is_completed=0  Group By order_no');
                    while ($row = mysqli_fetch_array($q)) {
                        ...
                        <tr>
                            ...
                            <td><span class="label label-success" style=" padding: 3px 15px;"><?php echo "Pending"; ?></span></td>
                            <td class="text-center">
                                <div role="group" aria-label="Basic example" class="btn-group btn-group-sm">
                                    ...
                                    <button type="button"   class="btn btn-outline btn-success" onclick="window.location.href = 'change_order_status.php?id=<?php echo $row['id']; ?>&page=1'"><i class="ti-pencil"></i></button>						
